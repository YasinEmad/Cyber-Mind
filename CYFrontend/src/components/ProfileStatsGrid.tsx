...
const ProfileStatsGrid: React.FC<ProfileStatsGridProps> = ({ user }) => (
  <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <StatBox icon={<CheckSquare />} label="Completed" value={user?.profile?.challengesDone?.toString() || '0'} color="text-blue-400" />
    <StatBox icon={<Puzzle />} label="Puzzles" value={user?.profile?.puzzlesDone?.toString() || '0'} color="text-purple-400" />
    <StatBox icon={<Flag />} label="CTFs Solved" value={((user?.profile?.solvedCTFLevels?.length || 0) || (user?.solvedChallenges?.length || 0)).toString()} color="text-red-400" />
  </section>
);
