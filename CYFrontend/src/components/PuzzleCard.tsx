...
const PuzzleCard: React.FC<PuzzleCardProps> = ({ puzzle, index }) => {
  ...
  const scrambleTitle = () => {
    ...
    intervalRef.current = window.setInterval(() => {
      ...
      if (iteration >= puzzle.title.length && intervalRef.current) {
    ...
    >
      ...
      <div className="p-6 flex flex-col h-full relative z-10">
        <div className="flex-grow">
          <span className="inline-block px-3 py-1 text-xs font-semibold rounded-full border bg-purple-600/30 text-purple-300 border-purple-500/50">
            {puzzle.category}
          </span>
        ...
        <div className="mt-6">
          <div className="w-full flex items-center justify-center px-4 py-2 bg-slate-700 text-slate-200 font-semibold rounded-lg group-hover:bg-purple-600 group-hover:text-white transition-colors duration-300">
